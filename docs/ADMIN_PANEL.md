...
> **Забележка:** Изтриването на потребители е умишлено премахнато от интерфейса. За да изтриеш потребител, използвай Supabase Dashboard → Authentication → Users.
...
Връща дневни регистрации за последните **30 дни** — всеки ден е включен дори при 0 регистрации:
```js
[{ date: '2026-02-01', count: 3 }, ...] // 30 елемента
```
...
След последни промени са приложени следните мерки:

| Мярка | Описание |
|-------|----------|
| `admins` таблица с RLS | Директен client достъп е блокиран — само server-side/service role |
| `users` INSERT политика | Само логнат потребител може да вмъква собствен профил (`auth.uid() = auth_user_id`) |
| `service_logs` политики | Пълен CRUD само за записи, свързани с коли на текущия потребител |
| Storage `documents` политики | Коригирани — старите никога не съвпадаха (проверяваха UUID срещу integer ID) |
| Function `search_path` | `is_admin`, `invoke_check_reminders`, `update_updated_at_column` имат фиксиран `search_path = public` |

> Всички промени са приложени чрез миграции и не изискват ръчна намеса.

---

## Изтриване на потребители

Изтриването на потребители от admin интерфейса е **умишлено премахнато**, тъй като изисква service role key (не може безопасно от клиента).

За да изтриеш потребител:
1. **Supabase Dashboard** → Authentication → Users → намери потребителя → Delete
2. Записите в `public.users`, `cars` и `services` ще бъдат изтрити автоматично чрез `ON DELETE CASCADE` (ако е конфигурирано), или ръчно:

```sql
-- Изтриване на всички данни на потребител по email
